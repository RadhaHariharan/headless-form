...
run(`pnpm --filter @headlesskit/forms run build`, 'Build core');
run(`pnpm --filter @headlesskit/forms-react run build`, 'Build react');
// Angular build requires ng-packagr; skip in CI if not available.
try {
  run(`pnpm --filter @headlesskit/angular run build`, 'Build angular');
} catch {
