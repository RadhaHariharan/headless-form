...
import type { UseFormOptions } from '@headless-form/core';
 ...
 *
 * @remarks
 * Identical in behaviour to Mantine's `createFormContext`. Nested providers work independently;
 * each provider's context is isolated to its subtree.
 *
 * @typeParam Values - The form values type.
...
>(): [
  ...
  function useFormContext(): UseFormReturnType<Values, TransformedValues, TError> {
    ...
    if (ctx === null) {
      throw new Error(
        '[headless-form/useFormContext] No FormProvider found in the component tree. ' +
