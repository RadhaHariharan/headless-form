 ...
 *
 * @remarks
 * - In **controlled** mode, `values` is reactive React state — every `setFieldValue` triggers a re-render.
 * - In **uncontrolled** mode, only error/status state changes trigger re-renders; typing does not.
...
export function useForm<Values = Record<string, unknown>, TransformedValues = Values, TError = ReactNode>(
  ...
  if (storeRef.current === null) {
    storeRef.current = createFormStore<Values, TransformedValues, TError>(options);
  }
