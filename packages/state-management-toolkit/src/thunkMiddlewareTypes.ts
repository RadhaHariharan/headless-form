import type { Action, AnyAction, Middleware } from '@headlesskit/state-management'

/**
 * The dispatch method as modified by the thunk middleware; overloaded so that you can
 * dispatch:
 *   - standard (object) actions: `dispatch()` returns the action itself
 *   - thunk actions: `dispatch()` returns the thunk's return value
 *
 * @template State The store's state
...
/**
 * A "thunk" action (a callback function that can be dispatched to the
 * store.)
 *
 ...
 */
export type ThunkMiddleware<State = any, BasicAction extends Action = AnyAction, ExtraThunkArg = undefined> =
